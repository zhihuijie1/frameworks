...
(3)所以，现在代码在编写的过程中存在的问题是：==耦合度偏高== （**一个动，整个项目都要动**）
...
```
IOC:控制反转 - 主要是将创建对象的权力由自己交给了外部，当想要使用对象时，不是自己new一个，而是从IOC容器中直接注入
IOC容器：用来存放控制反转所创建对象的一个池子
bean：ioc控制反转所创建的对象称之为bean
DI：依赖注入 - ioc容器中各个bean对象之间是有依赖关系的，然后在IOC容器中的bean对象之间建立这种依赖关系成为依赖注入。
```
